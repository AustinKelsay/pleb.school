...
- In production deployments, `src/lib/env.ts` enforces a fail-fast required env contract (`DATABASE_URL`, `NEXTAUTH_SECRET`, `NEXTAUTH_URL`, `PRIVKEY_ENCRYPTION_KEY`, `KV_REST_API_URL`, `KV_REST_API_TOKEN`, `VIEWS_CRON_SECRET`) and rejects insecure/malformed values (for example non-HTTPS `NEXTAUTH_URL`). Vercel previews (`VERCEL_ENV=preview`) still validate core DB/auth secret requirements while allowing preview-optional keys (`NEXTAUTH_URL`, `KV_REST_API_URL`, `KV_REST_API_TOKEN`, `VIEWS_CRON_SECRET`) to be omitted.
